...
function initializeVonage(data, videoTrack, audioTrack) {
    ...
    // Listen for the webhook relay from our backend
    vonageSession.on('signal:archiveAvailable', (event) => {
        const downloadUrl = event.data;
        const container = document.getElementById('archiveLinkContainer');
        const link = document.getElementById('archiveDownloadLink');
        const timerText = document.getElementById('archiveTimer');
        
        link.href = downloadUrl;
        container.style.display = 'flex';
        
        let timeLeft = 600; // 10 minutes (Vonage URL expiration)
        const countdown = setInterval(() => {
            timeLeft--;
            const minutes = Math.floor(timeLeft / 60);
            const seconds = timeLeft % 60;
            timerText.innerText = `Expires in: ${minutes}:${seconds.toString().padStart(2, '0')}`;
...
//         const res = await fetch('/api/composer/start', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: vonageSession.sessionId, roomName: ui.roomName.value }) });
...
ui.startBroadcastBtn.addEventListener('click', async () => {
    if (activeBroadcastId) {
        ...
        // Generate and display the watch page link dynamically
        const watchUrl = `${window.location.origin}/watch.html?room=${ui.roomName.value.trim()}`;
        document.getElementById('shareLinkText').innerText = watchUrl;
        document.getElementById('shareLinkText').href = watchUrl;
...
ui.startArchiveBtn.addEventListener('click', async () => {
    if (activeArchiveId) {
        await fetch('/api/archive/stop', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ archiveId: activeArchiveId, sessionId: vonageSession.sessionId }) });
